...
public class AnimationReader {
   ...
   * alphabetic strings with no spaces), giving the unique name of the shape, and
   * the type of shape it is.</li>
   * <li>Motion lines: the keyword "motion" followed by an identifier giving the
   * name of the shape to move, and 16 integers giving the initial and final
   * conditions of the motion: eight numbers giving the time, the x and y
   * coordinates, the width and height, and the red, green and blue color values
   * at the start of the motion; followed by eight numbers for the end of the
   * motion. See {@link AnimationBuilder#addMotion}</li>
   * </ul>
   * </p>
   *
   ...
   * @return
   */
  public static IAnimationModel parseFile(Readable readable, AnimationBuilder builder) {
  ...
  /**
   * Read the canvas from the file and set the bound.
   * 
   * @param s       scanner
  ...
  /**
   * Read the shape from the file and declare the name and type of the shape.
   * 
   * @param s       scanner
   * @param builder animation builder
   */
  ...
  private static void readMotion(Scanner s, AnimationBuilder builder) {
    ...
    builder.addMotion(name, vals[0], vals[1], vals[2], vals[3], vals[4], 
  ...
  private static int getInt(Scanner s, String label, String fieldName) {
    if (s.hasNextInt()) {
      ...
      throw new IllegalStateException(
          String.format("%s: expected integer for %s, got: %s", 
                        label, 
                        fieldName, 
                        s.next()));
    } else {
